...
# don't log tensorflow warnings and info messages
...
top_model_weights_path = 'model_live.weights.h5'
...
def save_bottlebeck_features():
    ...
    np.save(open('bottleneck_features_train_live.npy', 'wb'),
    ...
    np.save(open('bottleneck_features_validation_live.npy', 'wb'),
            bottleneck_features_validation)


def train_top_model():

    train_data = np.load(open('bottleneck_features_train_live.npy', 'rb'))
    train_labels = np.array(
        [0] * (int(nb_train_samples / 2)) + [1] * (int(nb_train_samples / 2)))

    validation_data = np.load(open('bottleneck_features_validation_live.npy', 'rb'))
